<script lang="ts">
	...
	import { productsConfig } from '$lib/config/products.js';

	let pager: ReturnType<typeof getPagerForDoc>;

	function getPagerForDoc(slug: string) {
		const flattenedLinks = [null, ...flatten(productsConfig.sidebarNav), null];
		let activeIndex: number;
		if (!slug) {
			activeIndex = 1;
		} else {
			activeIndex = flattenedLinks.findIndex((link) => `/tool/${slug}` === link?.href);
		}
